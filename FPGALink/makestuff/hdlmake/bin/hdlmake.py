...
def addLibrary(hdlSet, baseDir, varMap):
    tree = yaml.load(open(baseDir + "/hdlmake.cfg"), yaml.BaseLoader)
    hdls = tree["hdls"]
...
def readHdlMake(baseDir, isRequired = True):
    ...
    if ( os.path.exists(hmFile) ):
        return yaml.load(open(hmFile, "r"), yaml.BaseLoader)
    elif ( isRequired ):
...
def appBuild(template, board):
    ...
    boardTree = yaml.load(open(boardDir + "/board.cfg"), yaml.BaseLoader)
    ...
    if ( dirname[:3] == "tb_" ):
        ...
        if ( argList.w and "signals" in tbTree ):
            ...
            if ( "sigmaps" in tbTree ):
                for (sigTag, sigMap) in list(tbTree["sigmaps"].items()):
